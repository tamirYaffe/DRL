...
CP_env_state_size = 4
CP_env_action_size = 2

AC_env = gym.make('Acrobot-v1')
AC_env_state_size = 6
AC_env_action_size = 3
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            self.W2 = tf.compat.v1.get_variable("AC_W2", [32, 1],
                                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0,
                                                                                                            ...
                                                                                                            distribution="uniform",
                                                                                                            seed=0))
            self.b2 = tf.compat.v1.get_variable("AC_b2", [1], initializer=tf.compat.v1.zeros_initializer())
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            self.W2 = tf.compat.v1.get_variable("AC_W2", [32, self.action_size],
                                                initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0,
                                                                                                            ...
                                                                                                            distribution="uniform",
                                                                                                            seed=0))
            self.b2 = tf.compat.v1.get_variable("AC_b2", [self.action_size], initializer=tf.compat.v1.zeros_initializer())
...
ACpolicy = ACPolicyNetwork(global_state_size, global_action_size, learning_rate)
ACstate_value_network = ACStateValueNetwork(global_state_size, learning_rate)
...
def remove_actions_padding(actions):
    if len(actions) == CP_env_action_size:
        return actions

    actions = actions[:CP_env_action_size]
...
with tf.compat.v1.Session() as sess:
    ...
    for episode in range(max_episodes):
        ...
        for step in range(max_steps):
            ...
            actions_distribution = sess.run(ACpolicy.actions_distribution, {ACpolicy.state: state})

            # remove padding from actions
            # actions_distribution = remove_actions_padding(actions_distribution)

            action = np.random.choice(np.arange(len(actions_distribution)), p=actions_distribution)
            if action >= AC_env_action_size:
            ...
            state_value_approx = sess.run(ACstate_value_network.state_value, {ACstate_value_network.state: state})

            # calculate approx_value = b(S't)
            if done:
                next_state_value_approx = 0
            else:
                next_state_value_approx = sess.run(ACstate_value_network.state_value,
                                                   {ACstate_value_network.state: next_state})
            ...
            if done:
                ...
                if average_rewards > -100 and episode > 98:
        ...
        for t, transition in enumerate(episode_transitions):
            ...
            actor_feed_dict = {ACpolicy.state: transition.state, ACpolicy.R_t: advantage,
                               ACpolicy.action: transition.action}
            _, actor_loss = sess.run([ACpolicy.optimizer, ACpolicy.loss], actor_feed_dict)

            critic_feed_dict = {ACstate_value_network.state: transition.state,
                                ACstate_value_network.R_t: total_discounted_return}
            _, critic_loss = sess.run([ACstate_value_network.optimizer, ACstate_value_network.loss],
                                      critic_feed_dict)
    ...
    for episode in range(max_episodes):
        ...
        if solved:
            # save models
            saver.save(sess, saved_models_dir + path_sep + 'CPAC' + path_sep + "CPAC_model")
            break
